...
import type { billGroups } from "@/lib/bill-data";
...
/* The monthly charts: a drawing, with a button per month laid over it. */
export const interactiveChart = "relative h-[260px] min-w-[900px]";
/** The drawings' Y axis; the month buttons start after it. */
export const chartAxisWidth = 44;
export const monthButtons = "absolute inset-y-0 right-0 grid grid-cols-12";
...
export type SeriesKey = (typeof billGroups)[number][0] | "paid";
/* Each line's colour, and a dash pattern so lines differ without colour. */
export const lineStyle: Record<
  SeriesKey,
  { color: string; width: number; dash?: string }
> = {
  energy: { color: "var(--chart-1)", width: 3 },
  power: { color: "var(--chart-2)", width: 3, dash: "7 3" },
  other: { color: "var(--chart-3)", width: 3, dash: "2 3" },
  taxes: { color: "var(--chart-4)", width: 3, dash: "9 3 2 3" },
  unknown: { color: "var(--chart-5)", width: 3, dash: "2 5" },
  credit: { color: "var(--destructive)", width: 3, dash: "6 3" },
  paid: { color: "var(--foreground)", width: 4 },
};
