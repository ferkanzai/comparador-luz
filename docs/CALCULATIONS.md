...
2. Power = (P1 kW × P1 price + P2 kW × P2 price) × billing days. If prices are in €/kW/year, divide by 365. For €/kW/month, multiply by 12 / 365. Monthly prices are annualized rather than assuming every month has 30 days; supplier-specific proration may differ. The invoice-amount price reconstruction uses the same conversion in reverse. This applies to all three power price modes.
...
## Recorded consumption and tariff checks
...
An optional invoice-specific power/tax profile enables a **non-blocking** tariff plausibility check. Invoice dates override snapshot days; invoice kWh override snapshot consumption. Period tariffs require all three kWh values. Fixed energy tariffs can use total-only kWh without storing an invented period distribution. Incomplete inputs produce no estimate. Itemized bills compare energy + power + financing + rental + services **before taxes**, avoiding false warnings from historical IVA/IEE rates. Total-only bills use explicitly entered invoice tax settings and subtract the recorded final credit; the app does not infer historical tax rules from a calendar cutoff.

A difference is flagged only when it exceeds `max(€2, 5% × abs(actual compared amount))`. This is a product tolerance, not proof of billing correctness. Candidate saved tariffs and history snapshots covering the entire invoice period are suggested only if within that tolerance and less than half as far from the actual amount as the selected tariff. They are price matches, not identification of the customer's contract. Explicit energy/power price lines suppress replacement suggestions because one tariff may not describe the full invoice. Discounts, charges or split periods can still explain differences.

Users can keep a mismatching tariff, choose an explanation, and save an acknowledgement bound to the relevant invoice inputs. Editing those inputs invalidates that acknowledgement. A warning never overwrites the paid amount or blocks saving; actual breakdown reconciliation and consumption consistency still do. Creating a new tariff from the invoice opens the existing price editor and holds a draft: it is saved atomically with the invoice only if still linked, and does not change the current contract. Canceling the invoice discards that draft.

## Comparing invoice years

The **Por años** section is available when saved invoices use at least two distinct reporting years. It compares any two recorded years, defaulting to the latest and its preceding recorded year, with paid-amount and consumption views. Monthly columns aggregate all invoices assigned to that reporting month, including net credits and negative paid totals. Missing months are not plotted as zero; explicit zero values remain known data.
