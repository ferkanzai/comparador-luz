...
- [ ] With no tariffs: one primary call to action. Hide or disable "Simular consumo" and "Exportar", and move the status message until there's data.
- [ ] Consider leading with the comparison profile ("Ten tu factura a mano") as the first step, if that matches the intended flow.
- [ ] Hide "Guardar este período como factura" until there's a current tariff with a cost.
- [ ] Merge the save strip and status into one compact line, and make the verification banner dismissible for the session.
- [ ] Check desktop and phone, as a guest and signed in.
